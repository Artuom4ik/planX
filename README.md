...
* В приложениие `links` есть кастомная команда для добавления тестовых данных, находящиеся по адресу `links/management/commands/create_test_data.py`
* Этот скрипт генерирует 12 случайных пользователей и от 5 до 15 ссылок к ним.
* Для запуска команды требуется в консоли написать команду:
